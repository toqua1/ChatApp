...
import 'package:chatapp/screens/chats/chatRoom.dart';
...
class _ChatCardState extends State<ChatCard>{
  ...
  Widget build(BuildContext context) {
    String userId=widget.item.members!.where((element) => element !=
        FirebaseAuth
        .instance.currentUser!.uid).first ; /*['id'] => 'id' */

    return StreamBuilder(
      stream: FirebaseFirestore.instance.collection('users').doc(userId).snapshots() ,
      builder:(context ,snapshot){
        // ChatUser? chatUser=ChatUser.fromJson(snapshot.data!.data()!);
        ChatUser? chatUser = snapshot.data?.data() != null ? ChatUser.fromJson(snapshot.data!.data()!) : null;
        if(snapshot.hasData){
         return InkWell(
           onTap: (){
